...
def main():
    parser = argparse.ArgumentParser(description="Futakotamagawaeki Tama11 bus timetable")
    parser.add_argument("--full", action="store_true", help="Print full schedule instead of next-hour departures")
    args = parser.parse_args()

    print(f"Fetching timetables for {STOP_NAME} ({ROUTE_FILTER})...")
    timetables = build_timetables(CONSUMER_KEY, STOP_NAME, route_filter=ROUTE_FILTER)
